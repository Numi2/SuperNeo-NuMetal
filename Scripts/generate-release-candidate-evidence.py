...
def build_evidence(args: argparse.Namespace) -> dict:
    ...
    return {
        ...
        "productionSecurityBoundaries": [
            ...
            "NumiSeal product, carry, and ZK formalization has a checked evidence-parametric end-to-end theorem scope, exact rejection-sampled field mask distribution evidence, proof-level ZK simulator coupling with epsilon_zk_sim = 0 under declared leakage, a selected-depth loss-accounting ledger, extractor loss accounting, QROM Fiat-Shamir accounting with explicit collision mapping and an instantiated conditional Q_H = 2^64 query cap, a QROM transcript schedule, conditional QROM sampler/encoding evidence under the QRO abstraction, QROM collision/malleability structural evidence, QROM transform preconditions, a QROM interactive reduction ledger with code-enforced NumiSeal challenge maxima, shared-core bad-event deduplication, a total-loss budget contract, a product release distribution evidence contract, and a product cryptographic security dossier pinned to bounded depth 1; concrete extractor implementation, residual source/terminal/extractor loss instantiation, recursive product carry flow, side-channel evidence, post-quantum parameter tightening, hosted operations, release signing, notarization/publication proof, hosted branch-protection evidence, archived release evidence, and remaining numeric loss instantiations remain production-security boundaries.",
        ],
    }
