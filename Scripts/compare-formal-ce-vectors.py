...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare executable Swift CE proof vectors against Lean grammar vectors."
    )
    ...
    swift_vectors = load_or_run(
        ...
        ["swift", "run", "-c", "release", "--package-path", str(root), "superneo-formal-vectors", "ce"],
        root,
        "Swift CE vector emitter",
    )
    lean_vectors = load_or_run(
        args.lean_output_file,
        ["lake", "env", "lean", "--run", "../Scripts/emit-formal-ce-vectors.lean"],
        root / "Formal",
        "Lean CE vector emitter",
    )
    ...
    if mismatches:
        fail("Swift/Lean CE vector mismatch:\n" + "\n".join(mismatches))

    print("Swift/Lean CE opening proof vectors match")
