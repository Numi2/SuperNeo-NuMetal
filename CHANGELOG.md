...
- Added a checked NumiSeal end-to-end theorem scope in Lean and release
  ...
  coverage.
- Added checked Lean theorem surfaces for recursive folding knowledge
...
- Added pinned local constant-time release evidence under
  ...
  `Scripts/generate-constant-time-release-evidence.py`, covering Metal
  AIR/metallib artifacts, runtime allocation/COW static review, and CPU/GPU
  observation corpora plus compiler/hardware observation lane reports.
...
- Swift optimized SIL/LLVM/assembly review, hardware counters,
  power/contention, and broader device-lane evidence beyond the pinned
  compiler/hardware observation lanes in the local constant-time release
  evidence.
