...
abbrev Message (columns : Nat) := Fin columns → RF
abbrev AjtaiMatrix (rows columns : Nat) := Fin rows → Fin columns → RF
abbrev Commitment (rows : Nat) := Fin rows → RF

def commit {rows columns : Nat} (A : AjtaiMatrix rows columns) (z : Message columns) :
    Commitment rows :=
  fun row => ∑ col : Fin columns, A row col * z col

def Opening {rows columns : Nat}
    (A : AjtaiMatrix rows columns)
    (c : Commitment rows)
    (bounded : Message columns → Prop)
    (z : Message columns) : Prop :=
  bounded z ∧ commit A z = c

def DifferenceOfBounded {columns : Nat}
    (bounded : Message columns → Prop)
    (diff : Message columns) : Prop :=
  ∃ lhs rhs : Message columns, bounded lhs ∧ bounded rhs ∧ diff = lhs - rhs

def NoShortKernel {rows columns : Nat}
    (A : AjtaiMatrix rows columns)
    (bounded : Message columns → Prop) : Prop :=
  ∀ diff : Message columns,
    DifferenceOfBounded bounded diff →
    commit A diff = 0 →
    diff = 0

theorem commit_zero {rows columns : Nat} (A : AjtaiMatrix rows columns) :
    commit A (0 : Message columns) = 0 := by
  funext row
  simp [commit]

theorem commit_add {rows columns : Nat}
    (A : AjtaiMatrix rows columns)
    (lhs rhs : Message columns) :
...
theorem binding_from_noShortKernel {rows columns : Nat}
  ...
  funext col
  have hAtCol : (lhs - rhs) col = (0 : RF) := by
    exact congrFun hDiffZero col
