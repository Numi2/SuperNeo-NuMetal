...
- Set the manifest current label and tracked documentation claims to
  `partial formalization`.
- Updated status-bearing documentation to use the partial label and state that
  PiDEC, PiRLC, PiCCS, terminal CE, and full SuperNeo composition remain open.
...
- Extended the regression harness to prove that the completed-protocol label is
  still blocked by planned theorem groups and that documentation overclaims are
  rejected.

## Residual Boundaries

- This is not a production cryptographic certification.
- This is not a formal proof of the full SuperNeo protocol, terminal CE
  relation, PiDEC recomposition, PiRLC soundness, PiCCS sum-check model, or
...
- Local validation in this workspace could not run `lake build` because Lean/Lake
  were not installed in the shell environment. The GitHub formal workflow remains
  the authoritative Lean-build gate for formal changes.
