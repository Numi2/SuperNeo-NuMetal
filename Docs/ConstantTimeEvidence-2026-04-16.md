...
- `Evidence/ConstantTime/swift-llvm-metal-v1/manifest.json` pins the local
  release evidence generated for this slice: Swift optimized SIL, Swift
  optimized LLVM IR, Swift target assembly, Swift compiler generation report,
  Metal AIR, linked metallib, Metal generation report, Swift runtime
  allocation/COW static review, CPU observation corpus, direct GPU kernel
  observation corpus, compiler observation lane report, and hardware observation
  lane report.
...
- `swift/GoldilocksField.optimized.sil`: optimized SIL emitted from the checked
  `GoldilocksField.swift` source with the release Swift frontend.
- `swift/GoldilocksField.optimized.ll`: optimized LLVM IR emitted from the
  checked `GoldilocksField.swift` source with the release Swift frontend.
- `swift/GoldilocksField.arm64.s`: target assembly emitted from the checked
  `GoldilocksField.swift` source with the release Swift frontend.
- `swift/swift-compiler-artifacts-v1.json`: Swift source digest, toolchain
  versions, compile commands, output digests, covered Swift regions, and
  residual scoped-review boundaries.
...
- `swift-llvm-goldilocks-arithmetic`: the Swift source/runtime review is pinned,
  and optimized SIL, LLVM IR, and target assembly are pinned. Scoped review of
  those emitted artifacts is still required before production constant-time
  promotion.
...
- Review the emitted SIL/LLVM and target assembly for Swift Bool-to-mask and
  select lowering, or replace the helper with a verified primitive.
