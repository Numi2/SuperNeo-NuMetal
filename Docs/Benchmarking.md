...
The artifact format is rendered by `Scripts/render-paper-reproduction.swift`.
...
- `Docs/BenchmarkReports/README.md` is the checked-in report index.
- Fresh `benchmark-results/report.md`, `benchmark-results/results.json`, and
  `benchmark-results/metadata.json` files are generated by
  `Scripts/run-benchmarks.sh` and should be attached to release/audit evidence
  when hardware timing claims are made.
...
the current constant-time and Metal evidence notes in
