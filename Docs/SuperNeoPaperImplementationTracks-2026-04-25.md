...
| --- | --- | --- |
| Paper reproduction docs | Missing DocC/script docs have replacement entry points. | `Scripts/reproduce-superneo-paper.sh plan` |
...
| Parameter/security model | Parameter notes and estimator scripts exist; paper entry docs now point to them. | Add sensitivity matrix rows for each selected profile parameter. |
...
| Ideal QRO vs concrete hash | QRO product architecture is explicit; concrete deployment remains unpromoted. | Add deployment-domain inventory and concrete-hash attack analysis. |
